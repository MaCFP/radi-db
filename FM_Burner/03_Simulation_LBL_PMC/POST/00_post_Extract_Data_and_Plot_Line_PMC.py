...
# ------------------------------------------------------------------------------
# unitsFluidRADL = ["G","1/m"]
unitsFluidRADL = ["1/m","kg/ms^3","kg/ms^3","-","1/m","kg/ms^3","1/m"]
unitsWallRADL = ["qin","qr","qem"]
# unitsInitL = ["-","-","-","-","-","-","K","kg/m3","soot"]
unitsInitL = ["kg/m3"]
...
for i, case in enumerate(caseL):
    ...
    for n, csvL in enumerate(csvAllL):
        ...
        for m, csv in enumerate(csvL):
            print (100*"=")
            print("csv: ",csv)
            globPattern = case+"/postProcessing/"+csv+"/*"
            print("globPattern: ",globPattern)
            timeL = glob.glob(globPattern)
            ...
            plt.savefig(postDir1 +"/"+ csv+"_ALL.png")
            plt.close()
            ...
            for p,colN in enumerate(df.columns):
                ...
                df[colN].to_csv(postDir0 +"/PMC_LBL_"+rootCase +"__"+ csvNew +"__"+colN+".csv")
